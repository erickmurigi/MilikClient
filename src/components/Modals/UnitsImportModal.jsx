    ...
    try {
      ...
      if (result?.data) {
        ...
        if (successful.length > 0 && failed.length === 0) {
          toast.success(`Successfully imported ${successful.length} units!`);
        } else if (successful.length > 0 && failed.length > 0) {
          toast.warning(`Imported ${successful.length} units. ${failed.length} failed.`);
          // Show first few failures as details
          if (failed.length > 0) {
            const failureMsg = failed.slice(0, 3).map(f => `${f.unitNumber}: ${f.error}`).join('\n');
          ...
          console.error('All units failed to import:');
          failed.forEach(f => console.error(`  ${f.unitNumber}: ${f.error}`));
        }
      } else {
        toast.success(`Successfully imported ${parseResult.validCount} units!`);
      }

      handleClose();
    } catch (error) {
      console.error('Import error in modal:', error);
      toast.error(error.message || 'Failed to import units. Please check console for details.');
